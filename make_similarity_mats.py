'''
compute similarity matrices using EMA data
per subject and syllable group
using only common words across subjects (without stopwords)
'''
...
from utils.preprocess_data import filter_by_common, separate_by_syl, pad_data, compute_word_difference
...
def main(subjects, n_syllables):
    ...
    for subject in subjects:
        ...
        frames = filter_by_common(frames)
        ...
        for n_syl in n_syllables:
            ...
            syl_frames = separate_by_syl(frames, n_syl, normalize)

            # word_inds = [i for i in frames.keys() if frames[i].word == 'organizations']
            ...
            # plot
            plot_correlation_image(subject, correlations, large=True if n_syl < 4 else False)
            plt.savefig(f'pics/{subject.lower()}_similarity_mat_normalize_{str(normalize).lower()}_nsyl{str(n_syl)}_sorted.png', dpi=320)
            plt.close()

            # save
            correlations.to_csv(f'results/{subject.lower()}_similarity_mat_normalize_{str(normalize).lower()}_nsyl{str(n_syl)}.csv',
...
if __name__ == '__main__':
    ...
    main(args.subjects, args.syllables)